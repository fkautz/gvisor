...
func saveOptsFromSpec(spec *specs.Spec, fds []*fd.FD, useCheckpointGofer bool) (_ *control.SaveOpts, retErr error) {
	...
	} else if len(files) != 3 && len(files) != 4 {
		return nil, fmt.Errorf("local workload checkpoint got %d files, want runtime, metadata, pages, and optional base", len(files))
	}
...
func (r *restorer) restore(l *Loader) error {
	...
	// Refresh the control server with the newly created kernel and ensure a
	// fresh acceptor is available after the restore transition.
	l.ctrl.refreshHandlersAfterRestore()
