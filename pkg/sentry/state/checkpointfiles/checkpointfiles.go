...
const (
	...
	// SharedBaseFileName is the exported shared MemoryFile base.
	SharedBaseFileName = "base.img"

	// CasimirLayoutFileName is the authoritative per-address-space LLML2
	// geometry sidecar associated with SharedBaseFileName.
	CasimirLayoutFileName = "casimir_layout.img"
...
func PlaneForFile(name string) (Plane, bool) {
	...
	case PagesMetadataFileName, PagesFileName, SharedBaseFileName, CasimirLayoutFileName:
...
func LocalStagingFileName(name string) (string, error) {
	switch name {
	case StateFileName, PagesMetadataFileName, PagesFileName, SharedBaseFileName, CasimirLayoutFileName:
		return "." + name + ".tmp", nil
	default:
...
// the shared-base and authoritative Casimir-layout FDs in that order. A
// four-FD legacy transaction containing only shared-base remains accepted.
// saveErr is returned together with any durable rollback failure.
func FinishLocalTransaction(dirFD int, planeFDs []int, saveErr error) error {
	...
	if len(planeFDs) == 4 {
		names = append(names, SharedBaseFileName)
	} else if len(planeFDs) == 5 {
		names = append(names, SharedBaseFileName, CasimirLayoutFileName)
	} else if len(planeFDs) != 3 {
		return errors.Join(saveErr, fmt.Errorf("got %d local checkpoint planes, want 3, 4, or 5", len(planeFDs)))
	}
	if saveErr != nil {
