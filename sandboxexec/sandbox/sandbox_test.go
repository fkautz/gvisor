...
func TestSandboxCommand(t *testing.T) {
	...
	sb, err := sandbox.New(ctx,
		sandbox.WithNetworking(false),
		sandbox.WithCommand("sleep", "1234"),
	)
	if err != nil {
		t.Fatalf("failed to start sandbox: %v", err)
	}
	...
	if !strings.Contains(string(data), `"sleep",`) || !strings.Contains(string(data), `"1234"`) {
		t.Errorf("config.json does not contain custom command. got: %s", string(data))
	}
}
